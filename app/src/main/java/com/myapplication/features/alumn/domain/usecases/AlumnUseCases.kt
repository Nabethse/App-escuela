...
class GetAlumnsUseCase @Inject constructor(private val repository: AlumnRepository) {
    suspend operator fun invoke(token: String) = repository.getAlumns(token)
}

class CreateAlumnUseCase @Inject constructor(private val repository: AlumnRepository) {
    suspend operator fun invoke(token: String, alumn: AlumnDto) = repository.createAlumn(token, alumn)
}

class UpdateAlumnUseCase @Inject constructor(private val repository: AlumnRepository) {
    suspend operator fun invoke(token: String, id: Int, alumn: AlumnDto) = repository.updateAlumn(token, id, alumn)
}

class DeleteAlumnUseCase @Inject constructor(private val repository: AlumnRepository) {
